...
public class ProcessBasedEnergetics implements PorpoiseEnergetics {
	...
	double storageLevelDelta;   // Change in storageLevel in the tick (in updStateVariables)
	double storageLevelDeltaSum;   // Change in storageLevel in the tick (in updStateVariables)
...
//	double ageDays;             // age in days
	...
	private double IRTimestep;  // FIXME JONAS ADDED
	double IRRecordSum;         // Sum of IRRecord. Reset to 0 every day
	public CircularBuffer<Double> irrecordDaily;   // List with average IRRecord for last ten days. Latest days first.
	...
	// counters
	int abortionCount = 0;         // abortion counter

	// For testing:
	double dailyFood;
	List<Double> foodIntakeList = new ArrayList<>();
	double energyExpenditureSum;     // FIXME Added by Jonas
	double energyExpenditureDaily;   // FIXME Added by Jonas
	double energyExpenditureTick;     // FIXME Added by Jonas
	double foodEatenDailyTemp;       // FIXME Added by Jonas
	double foodEatenDaily;       // FIXME Added by Jonas
	double foodEatenTick;       // FIXME Added by Jonas
	double foodEatenCalfTick;       // FIXME Added by Jonas
...
//		if (weanScaleFact < 1.0d && SimulationTime.getDayOfSimulation() < 90) {
		if (weanScaleFact < 1.0d && dsgBirth < 90) { // FIXME REMOVE AGAIN
			System.err.println("UNEXPECTED weanScaleFact: " + weanScaleFact);
		...
		EnergeticsDebugCapture.writePorp(this.porp);
		
		ExtendedEnergyDebugCapture.captureGeneral(porp.getId(), mTot, storageLevel, SLMean, vBlub, vBlubRepro, weight, porp.getAge(), calvesBorn, calvesWeaned, abortionCount);
		ExtendedEnergyDebugCapture.writeData();
	}
...
*/
		if (SimulationTime.isBeginningOfWeek()) {
			...
			dailyFood = (foodIntakeSum / 7.0d) * SimulationParameters.getIrToEA();
			foodIntakeList.clear();
		}
		
		final double sMean = this.storageLevelSum / 48.0d;
		this.storageLevelDaily.add(Math.round(sMean * 1000.0) / 1000.0);
//		System.out.println("storageLevelDaily " + storageLevelDaily.get(0));
		
//		double eeMean = this.energyExpenditureSum / 48.0d;
//		this.energyExpenditureDaily = Math.round(eeMean * 1000.0) / 1000.0;
		this.energyExpenditureDaily = this.energyExpenditureSum;
		this.energyExpenditureSum = 0.0d;
		
		this.foodEatenDaily = this.foodEatenDailyTemp;
		this.foodEatenDailyTemp = 0.d;

		final double irrMean = this.IRRecordSum / 48.0d;
	...
	public ProcessBasedEnergetics(Porpoise porp, boolean initialPopulation) {
		...
		if (initialPopulation) {
			if (porp.getAge() > porp.getAgeOfMaturity()) {
...
//			System.out.println(porp.getId() + " pregnancyStatus: " + pregnancyStatus);
				...
				if (this.pregnancyStatus == 2
					...
					this.dsMating = Globals.getRandomSource().getInitialDaysSinceMating();
...
//			System.out.println(porp.getId() + "pregnancyStatus: " + pregnancyStatus);
	...
	public ProcessBasedEnergetics(ProcessBasedEnergetics me) {
		...
		// FIXME Added by Jonas
		IRTimestep = 0.0d;
		IRRecordSum = 0.0d;
		irrecordDaily = new CircularBuffer<>(10);
		for (int i = 0; i < 10; i++) {
			this.irrecordDaily.add(0.0d);
		}
	...
	private void jonasInitReproductiveCosts() {
		/*
	...
	// New for energetics
	/*
	 * STATUS: Dev complete, whitebox tested
	 */
  ...
  [
    set swim-speed (mean-disp-dist * 1000 * (1 /(30 * 60)))                             ; convert dispersal distance in km 30min-1 to m s-1
		...
		if (!porp.getDispersalBehaviour().isDispersing()) {
			...
			// NEW FROM CARA FIXME
			double swimSpeedPresMov = (Math.pow(10.0d,  porp.getPresLogMov()) * 100.0d) / 1800.0d;
			swimSpeedPresMov = Math.log10(swimSpeedPresMov);
			double logSwimSpeed = 0.207d * swimSpeedPresMov + 0.1961d;  
			this.swimSpeed = Math.pow(10.0d, logSwimSpeed);

			EnergeticsDebugCapture.writeSwimspeed(this.porp, swimSpeed);
	...
	/*
	 * STATUS: DevComplete + WhiteboxTested
	 */
...
*/
		...
		IRTimestep = IRStructMass + pregIRSup + lactIRSup;
		IRRecord = IRRecord + IRTimestep;
		IRRecordSum += IRRecord;
		if (withLactCalf && weanScaleFact < 1.0d) {
		...
		double split = 0.0d;
		double foodAvailable = Globals.getCellData().getFoodLevel(ndPointToGridPoint(this.porp.posList.get(1)));  // FIXME Original states that it should be item 0 but uses item 1!
...
*/
		...
		foodEatenTick = IRReal;
		foodEatenCalfTick = IRRealCalf;
		foodEatenDailyTemp += IRReal; // + IRRealCalf;
		energyExpenditureSum += mTot / SimulationParameters.getIrToEA(); //IRTimestep;
		...
		eAssim = (IRReal * SimulationParameters.getIrToEA() * Globals.AEFood);
		eAssimCalf = (IRRealCalf * SimulationParameters.getIrToEA() * Globals.AEFood);
/*
  ; Debug energy intake
  if precision sim-day 3 = floor sim-day [if (debug = 11) [ if (who = 0) [ if food-available > 0 [
    ...
    print ""
		...
		ExtendedEnergyDebugCapture.captureEnergyIntake(IRRecord, foodAvailable, IRReal, IRRealCalf);
	}

	/*
	 * STATUS: DevComplete except for TODOs and FIXME below, Whitebox tested
	 */
...
//				System.out.println("DIE");
		...
		ExtendedEnergyDebugCapture.captureMaintenance(mBMR);
	}

	/*
	 * STATUS: DevComplete except FIXMEs, whitebox tested (allocation part)
	 */
...
//				System.out.println("die thermo");
		...
		ExtendedEnergyDebugCapture.captureThermoregulation(mThermo);
	}

	/*
	 * STATUS: DevComplete, whitebox tested
	 */
  ...
  ifelse e-assim >= m-loco                                                                           ; Check if the energy assimilated is sufficient to cover locomotive costs
    ...
    [ set e-storage (v-blub-min * dens-blub * ED-lip) ]
      ...
      ; For now this does not slow down swimming speed, but could in the future
		...
		ExtendedEnergyDebugCapture.captureLocomotion(mLoco, swimSpeed);
	}
...
*/
  ...
  ; run upd-blubber-depths to initialize blubber values
	...
	//calfEnergetics.porpsSetupParams(); // FIXME ADDED BY JONAS, Ok?
		...
		// elsewhere
		Globals.getMonthlyStats().addBirth(); // FIXME This doesn't seem to be in the right place
		calvesWeaned = calvesWeaned + 1;
  ...
  ; reset mother lactation variables
	...
	public void setPorp(Porpoise porp) {
		this.porp = porp;
	}

	/*
	 * Status: DevComplete, Whitebox tested ok
	 */
	private void growth() {
/*
to growth
   ...
   if mass-struct < m-str-inf                                                                                  ; If smaller than max female mass
      [
*/
		double growthRate = 0.0d; // FIXME Only moved to out here for debug capture.
...
*/
			...
			maxGrow = (mStrK / 17280.0d) * ((Math.pow(mStrInf, (1.0d / 3.0d)) * Math.pow(massStruct, (2.0d / 3.0d))) - massStruct);
...
*/
			...
			if (eAssimGrow >= mGrowth) {
				...
				growthRate = maxGrow;  // FIXME Not used?? Confirmed by Cara
			} else {
/*
        [
...
*/
				if (vBlub >= vBlubMean) {
					...
					eAssimGrow = 0.0d;
					massStruct = massStruct + maxGrow;
					growthRate = maxGrow;  // FIXME Not used?
				} else {
					growthRate = (eAssimGrow / EDLeanMass) * DELeanMass;
...
/*
  ...
  */
		...
		ExtendedEnergyDebugCapture.captureGrowth(maxGrow, mGrowth, growthRate, massStruct, lgth);
	}

	/*
	 * STATUS: DevComplete, Whitebox tested ok, except fixme
	 */
 ...
 */
		if (eAssim >= 0.0d) {
			double addBlub = ((eAssim * DELip * percLipBlub) / (Globals.EDLip * Globals.densBlub ));  // FIXME Check this
			vBlub = vBlub + addBlub;
			eAssim = 0.0d;
		}
 ...
 */
		...
		double storageLevelNew = (weight - massStruct) / weight;
		double storageLevelDelta = storageLevel - storageLevelNew;
		storageLevel = storageLevelNew;
	...
	/**
	 * STATUS: DevComplete, code reviewed
	 */
...
//		System.out.println("pregnancyStatus: " + pregnancyStatus);
...
*/
		if (this.pregnancyStatus == 2 && SimulationTime.getDayOfYear() == this.porp.getMatingDay()) {
			...
			if (pregChance == 0.0d) {
//			if (pregChance < 0.001d) {
				pregChance = Globals.getRandomSource().nextDouble();  // FIXME Move to separate method?
			}
/*
    ...
    [ if ( debug = 9 ) [print word who " aborted calf"]                                                                  ; If not, then abort calf
		...
		if (eAssim >= mPreg) {
			...
			if (eStorage >= mPreg + eRepoMin) {
				...
				pregnancyStatus = 0;
				mPreg = 0.0d;
				abortionCount = abortionCount + 1;
				Globals.nCalfLost = Globals.nCalfLost + 1;
				massF = 0.0d;
			}
		}
		ExtendedEnergyDebugCapture.capturePregnancy(pregnancyStatus, dsMating, massF, mGrowthF, eHeatGest, mPreg);
	}
...
*/
		if (massCalf <= 0.0d) {
			resetLactationVars();  // FIXME REPLACE THIS WITH check-calf-death  (there is an if condition there)
			return;
		}
/*
  ;;;;; COST CALCULATION ;;;;;
...
*/
		final double waterTemp = Globals.getCellData().getTemperature(porp.getPosition());
		double eThermoCalf = ThermoregulationLookupTable.getInstance().getValueInBin(waterTemp, swimSpeed, (int) Math.round(weight), storageLevel);  // FIXME Values to use here!!!
/*
...
*/
		if (weanScaleFact < 1.0d) {  // FIXME Changed this from 1 to 1.0d 
...
/*
  if wean-scale-fact < 1 [calf-feed]                                                                                                    ; If calf is being weaned trigger calf feeding procedure
		...
		ExtendedEnergyDebugCapture.captureLactation(dsgBirth, massCalf, massStructCalf, IRRecordCalf, 
				vBlubCalf, mBMRCalf, lgthCalf, mThermoCalf,
				maxGrowCalf, mGrowthCalf, mBlubCalf, eCalf, 
				mLact, vitalCosts, vitalCostsCalf);
	}
...
/*
  ...
  if (storage-level-calf < 0.05) [
	...
	/*
	 * STATUS: DevComplete, Whitebox tested, except fixmes
	 */
	public boolean updMortality() {
		...
		final double ranMortality = Globals.getRandomSource().nextMortality(0, 1);
		ReplayHelper.print("porp-upd-mortality:{0}", ranMortality);
		// Introducing maximum age and Mortality due to by-catch
		if (ranMortality > dailySurvivalProb || this.porp.getAge()> SimulationParameters.getMaxAge()) {
			Globals.getListOfDeadAge().addLast((int) this.porp.getAge());
			Globals.getListOfDeadDay().addLast(SimulationTime.getDayOfSimulation());
			this.porp.die(this.porp.getAge() > SimulationParameters.getMaxAge() ? CauseOfDeath.OldAge : CauseOfDeath.ByCatch);
			return false;
		}
...
*/
		...
		if (ran >= stepSurvProb) {
			if (!withLactCalf || storageLevel <= 0.0d) {
				...
				porp.die(CauseOfDeath.LowBodyCondition); // FIXME Check which value to pass here!
			}
		}
/*
 ...
 */
	...
	public void consumeEnergy(double energyAmount) {
		// energy consumption is handled differently here
	}

	@Override
	public double getFoodEaten() {
		return foodEatenDaily;
	}

	public double getFoodEatenTick() {
		return foodEatenTick;
	}

	public double getFoodEatenCalfTick() {
		return foodEatenCalfTick;
	}

	public double getEnergyConsumedTick() {
		return IRTimestep;
	}

	@Override
	public double getEnergyConsumedDaily() {
		return this.energyExpenditureDaily;
	}
