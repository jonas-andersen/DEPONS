...
//	private static double[] meanMaxEntInQuarters = {0.515686364223653, 0.888541219760357, 0.841346010536882, 1}; // standardized average maxent level in each quarter
...
/*
  ...
  set gravity 9.8
		...
		/* This is initialized in the landscape loader listener
		Globals.IRTempMod = calculateIRTempMod();
		System.out.println("IRTempMod.setup: " + Globals.IRTempMod + " [" + SimulationTime.getDayOfYear() + "]");
		*/
	}

	/**
	 * STATUS: WhiteboxTested ok
	 */
	public static double calculateIRTempMod() {
		/*
		  let xxxxx ((1 / 6.3661) * mean-temp) * (180 / pi)
		  let yyyyy (cos xxxxx)
		  set IR-temp-mod ((1 / 5)* yyyyy + 1)
		  */
		var meanTemp = Globals.getCellData().calcMeanTemperature(); //18.0d;// 3.3878634476026197d; //14.0; // FIXME
		var xxxxx = ((1.0d / 6.3661d) * meanTemp) * (180.0d / Math.PI);
		var yyyyy = Math.cos(Math.toRadians(xxxxx));
		return ((1.0d / 5.0d)* yyyyy + 1.0d);// Globals.calculateIRTempMod();
	}
	...
	public static double AEFood = 0.82d;
//	public static double IRToEA = 113750000;
	...
	public static Optional<MonthlyDataFile.Listener> dataFileListener = Optional.empty();
}
